...
public class HPBar : SaiMonoBehaviour
{
    ...
    protected virtual void HPShowing()
    {
        ...
        bool isDead = this.shootableObjectCtrl.DamageReciever.IsDead();
        if (isDead)
        {
            this.spawner.Despawn(transform);
            return;
        }
        float hp = this.shootableObjectCtrl.DamageReciever.HP;
        float maxHP = this.shootableObjectCtrl.DamageReciever.HPMax;
